...
gulp.task("resources", ['fonts', 'less'], () => {
  return gulp.src(["src/**/*", "!src/less", "!src/less/**/*", "!**/*.ts"])
    .pipe(gulp.dest(buildDir));
});
...
gulp.task('setting:dev', function(cb) {
  fs.writeFile('appsettings.yml', 'ApiEndpoint: http://dev.localhost/api/', cb);
});

gulp.task('setting:stage', function(cb) {
  fs.writeFile('appsettings.yml', 'ApiEndpoint: http://stage.localhost/api/', cb);
});

gulp.task('setting:prod', function(cb) {
  fs.writeFile('appsettings.yml', 'ApiEndpoint: http://prod.localhost/api/', cb);
});
